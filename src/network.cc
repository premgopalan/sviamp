...
{
  fprintf(stdout, "+ reading network %s\n", s.c_str());
  fflush(stdout);
  ...
  if (!f) {
    fprintf(stderr, "error: cannot open file %s:%s", s.c_str(), strerror(errno));
    exit(-1);
  }
  char b1[512], b2[512];
  string s1, s2;
  ...
  */
  fprintf(stdout, "+ done reading network\n");
  fflush(stdout);
  ...
  fprintf(stdout, "+ setting sparse zeros\n");
  fflush(stdout);

  if (_env.strid)
    printf("total nodes read = %d\n", scurr);
  
  if (_env.preprocess) {
    set_neighborhood_sets();
    exit(0);
  } else if (_env.informative_sampling && !_env.model_load && !_env.onesonly)
    load_neighborhood_sets();

  if (_env.groups_file != "")
    load_gt_groups();
  
  fprintf(stdout, "+ done setting sparse zeros\n");
  fflush(stdout);
  return 0;
}
...
{
  printf(".. loading init communities from %s\n", 
  ...
  printf("loaded %d init communities\n", cid);
  fflush(stdout);
  delete[] s;
  fclose(f);
...
void
Network::load_heldout_sets(string fname, SampleMap &mp, uArray &ignore_npairs)
{
  ...
  char buf[512];
  sprintf(buf, "loaded %s pairs:", fname.c_str());
