...
Network::get_edges(uint32_t a) const
{
...
// inline bool
// Network::exists(uint32_t p, uint32_t q) const
// {
// #ifndef SPARSE_NETWORK
//   assert (p < _y.m() && q < _y.m());
//   const yval_t **yd = _y.const_data();
//   return yd[a][b] & 0x01;
// #else
//   Edge e(p,q);
//   order_edge(_env, e);
//   if (e.first < _sparse_y.size()) {
//     const vector<uint32_t> *v = _sparse_y.at(e.first);
//     if (!v) {
//        fprintf(stderr, "Warning: node %d unexpectedly not "
//            found in sparse y\n", e.first);
//       return false;
//     } else {
...
//       if (!v.size())
// 	return 0;
//       for (uint32_t j = 0; j < _sparse_y.size(); ++j)
// 	if (e.second == v[j])
// 	  return 1;
//       return 0;
//     }
